...
export default function ZSCPage(props) {
    ...
    return (
        ...
        <Link className="nav-link active" aria-current="page" to={props.item.link}>
            ...
            <div className={isOwof ? 'kbP1024' : isOffz ? 'kbP1440' : isOntz ? 'kbP19206' : 'kbP'}>
                Rs {props.item.price}
            </div>
        ...
        <button onClick={() => props.Add(props.item)} className={isOwof ? 'bucketB1024' : isOffz ? 'bucketB1440' : isOntz ? 'bucketB19206' : 'bucketB'}>Add to bucket</button>
    </div>
