...
export default function TPage(props) {
    ...
    return (
        ...
        <Link className="nav-link active" aria-current="page" to={props.item.link}>
            ...
            <div className={isOwof ? 'kbP10244' : isOffz ? 'kbP14404' : isOntz ? 'kbP19204' : 'kbP'}>
                Rs {props.item.price}
            </div>
        ...
        <button onClick={() => props.Add(props.item)} className={isOwof ? 'bucketB1024' : isOffz ? 'bucketB1440' : isOntz ? 'bucketB19204' : 'bucketB'}>Add to bucket</button>
    </div>
