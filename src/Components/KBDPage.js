    ...
    return (
       ...
       <div className={isOwof ? 'y1024' : 'y'}>
        <Link className="nav-link active" aria-current="page" to={props.item.link}>
            ...
            <div className={isOwof ? 'kbP1024' : isOffz ? 'kbP14403' : isOntz ? 'kbP19203' : 'kbP'}>
                Rs {props.item.price}
            </div>
        ...
        <button onClick={() => props.Add(props.item)} className={isOwof ? 'bucketB1024' : isOffz ? 'bucketB1440' : isOntz ? 'bucketB19203' : 'bucketB'}>Add to bucket</button>
    </div>
