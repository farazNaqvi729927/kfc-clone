    ...
    return (
        <div className={isOwof ? 'y1024' : 'y'}>
            <Link className="nav-link active" aria-current="page" to={props.item.link}>
                ...
                <div className={isOwof ? 'kbP10241' : isOffz ? 'kbP1440' : isOntz ? 'kbP19201' : 'kbP'}>
                    Rs {props.item.price}
                </div>
            ...
            <button onClick={() => props.Add(props.item)} className={isOwof ? 'bucketB1024' : isOffz ? 'bucketB1440' : isOntz ? 'bucketB19201' : 'bucketB'}>Add to bucket</button>
        </div>
