...
export default function ZBage(props) {
    ...
    return (
        ...
        <Link className="nav-link active" aria-current="page" to={props.item.link}>
            ...
            <div className={isOwof ? 'kbP10245' : isOffz ? 'kbP14405' : isOntz ? 'kbP19205' : 'kbP'}>
                Rs {props.item.price}
            </div>
        ...
        <button onClick={() => props.Add(props.item)} className={isOwof ? 'bucketB1024' : isOffz ? 'bucketB1440' : isOntz ? 'bucketB19205' : 'bucketB'}>Add to bucket</button>
    </div>
